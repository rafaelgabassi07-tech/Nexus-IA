...
export const CodeBlock = React.memo(({ value, language = 'typescript', noMargin }: CodeBlockProps) => {
  ...
  return (
    <div className={cn("relative group h-full flex flex-col bg-[#0b0c0e]", !noMargin && "my-4 rounded-xl overflow-hidden border border-white/20 shadow-2xl")}>
      <div className="h-6 bg-black/40 border-b border-white/20 px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white/80 italic">Nexus Source View // {language}</span>
        </div>
        <div className="flex items-center gap-4 text-[8px] font-bold text-white/80 uppercase tracking-tighter italic">
      ...
      <div className="absolute right-4 top-10 z-30 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300">
        <button
          ...
          className="p-2.5 bg-black/40 backdrop-blur-md hover:bg-black/60 text-white/80 hover:text-white rounded-xl border border-white/30 transition-all shadow-xl active:scale-95"
        >
